...
class AsyncPropertyFuncStream(AbstractStream):
    """A stream that watches an async property stream for a change and produces a stream of functions to evaluate the property."""
    ...
    def __init__(self, stream):
        ...
        # initialize values
        self.__value = None
        self.__marked_dirty_listener = None

        # listen for display changes
        def update_value(new_value):

            def value_func():
                return new_value.get_value_immediate()

            self.__value = value_func if new_value else lambda: None

            if self.__marked_dirty_listener:
                self.__marked_dirty_listener.close()
                self.__marked_dirty_listener = None
            ...
            if new_value:
                ...
                self.__marked_dirty_listener = new_value.marked_dirty_event.listen(marked_dirty)
