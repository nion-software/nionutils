...
class ConcatStream(AbstractStream[T], typing.Generic[T]):
    """Make a new stream for each new value of input stream and concatenate new stream output."""

    def __init__(self, stream: AbstractStream[T], concat_fn: typing.Callable[[typing.Optional[T]], AbstractStream[T]]):
        super().__init__()
        # outgoing messages
        self.value_stream = Event.Event()
        # references
        ...
        self.__out_stream: typing.Optional[AbstractStream[T]] = None
        self.__out_stream_listener: typing.Optional[Event.EventListener] = None
        # define a stub and use weak_partial to avoid holding references to self.
        def stream_changed(stream: ConcatStream[T], value: typing.Optional[T]) -> None:
    ...
    def about_to_delete(self) -> None:
        ...
        if self.__out_stream:
            ...
            self.__out_stream = typing.cast(AbstractStream[T], None)
        self.__value = None
    ...
    def __stream_changed(self, item: typing.Optional[T]) -> None:
        if self.__out_stream_listener:
            self.__out_stream_listener.close()
            self.__out_stream_listener = None
        if self.__out_stream:
